...
const changePrivate = (ctx: CommandContext<Context & SessionFlavor<SessionData>>, mode: BotMessagingMode) => {
  ctx.session.messagingMode = mode;
  
  return ctx.reply("Режим применен!");
}

const changePublic = (ctx: CommandContext<Context & SessionFlavor<SessionData>>, mode: BotMessagingMode) => {
  if (!ctx.message) {
    return;
  }
  ...
  async (ctx: CommandContext<Context & SessionFlavor<SessionData>>) => {
    switch(ctx.chat.type) {
      ...
      case 'supergroup':
        return changePublic(ctx, mode);
      default:
        return ctx.reply(`Данная команда недоступна для типа чата: ${ctx.chat.type}`);
    }
  };
