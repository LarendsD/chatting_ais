...
"гендальф" → realistic`
  ...
  const prompt = promptTemplates[style as keyof typeof modelMap] || promptTemplates.realistic;
...
export const generateImage = async (
  ...
  denoiseStrength: number = 0.4, // Сила деноизинга для img2img (0-1, где 1 = полностью новая картинка). 0.4 лучше сохраняет исходное изображение
) => {
  ...
  const actualModel = inputImage 
    ? (IMG2IMG_MODEL_SELECTOR[style as keyof typeof IMG2IMG_MODEL_SELECTOR] || model)
    : model;
  ...
  if (inputImage && imageFilename) {
    ...
    workflow['5'] = {
      'inputs': {
        ...
        'denoise': denoiseStrength, // Используем переданную силу деноизинга (0.4 для сохранения исходного изображения)
        'model': ['4', 0],
        'positive': ['3', 0],
        'negative': ['6', 0],
